...
class DF(pd.DataFrame):
        ...
        if in_place:
            if row is not None:
                ...
                if isinstance(row, pd.Series):
                    ...
                    super().__init__(data=pd.concat([before, pd.DataFrame(row).transpose(), after], axis=0))
                else:  # pd.Dataframe
                    super().__init__(data=pd.concat([before, self._update_row_names(row), after], axis=0))
    ...
    #         self.__setattr__(name, value)  # instead, invoke setter on underlying pandas DataFrame
    #
    # def __getattribute__(self, name):
    #     try:
    #         return super(DF, self).__getattribute__(name)
    #     except AttributeError:  # don't immediately raise AttributeError
    #         return self.__getattribute__(name)  # instead, invoke getter on underlying pandas DataFrame
...
class RandomDF(DF):
    ...
    def regenerate(self,
        ...
        super().__init__(data=RandomDF(rows=number_of_rows if number_of_rows is not None else self.number_of_rows,
                                       ...
                                       distribution=distribution if distribution is not None else self.distribution).data)
